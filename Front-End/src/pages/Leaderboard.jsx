...
const Leaderboard = () => {
  ...
  const calculateRankings = (data) => {
    // Create a copy of the data array so we don't modify the original
    const participantsCopy = data.slice();

    // Sort participants by total points and badges (descending)
    const sortedData = participantsCopy.sort((a, b) => {
      // Primary sort: by total points (higher points = better rank)
    ...
    const rankedData = sortedData.map((participant, index) => {
      // If this participant has different points/badges than the previous one, update rank
      if (index > 0) {
      ...
      // Create a new participant object with rank added
      return {
        _id: participant._id,
  ...
  useEffect(() => {
    const fetchLeaderboardData = async () => {
      try {
        ...
        /* REAL BACKEND INTEGRATION - Uncomment when connecting:
        
        const response = await fetch('/api/leaderboard');
        const data = await response.json();
        const rankedData = calculateRankings(data);
                ...
                {leaderboardData.map((user, index) => (
                  <div
                    ...
                    className={`p-6 hover:bg-gray-50 transition-colors duration-200 ${
                      user.rank <= 3 ? 'bg-gradient-to-r from-blue-50 to-transparent' : ''
                    }`}
                  >
