...
function pathD(pts) {
  ...
  // Simple polyline converted to path
  return `M ${p.split(" ")[0]} L ${p.split(" ").slice(1).join(" ")}`;
}

const SQLiMap = () => {
  const { progress, awardStarsAndUnlock } = useWorldProgress("sqli");
  ...
  const handleLevelClick = (level, locked) => {
    ...
    window.location.href = `/quiz?vulnerability=sqli&level=${level}`;
  };

  return (
    <main className="min-h-screen w-full">
      <SEO
        title="SQL Injection World – Cybersecurity Galaxy Game"
        description="Play through 10 SQLi levels on a dotted galaxy path. Learn via tutorials and quizzes and earn stars to unlock the next planet."
        canonical="/sqli"
        jsonLd={{
          "@context": "https://schema.org",
          "@type": "VideoGame",
          name: "SQL Injection World",
      ...
      <section
        ...
        style={{ height: "1400px" }}
        aria-label="SQL Injection galaxy map"
      >
        <img
          src={bg}
