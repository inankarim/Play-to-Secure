...
const DOMClobberingMap = () => {
  const { progress, awardStarsAndUnlock } = useWorldProgress("domclobbering");
  ...
  const handleLevelClick = (level, locked) => {
    ...
    window.location.href = `/quiz?vulnerability=domclobbering&level=${level}`;
  };

  return (
    <main className="min-h-screen w-full">
      <SEO
        title="DOM Clobbering World – Cybersecurity Galaxy Game"
        description="Play through 10 DOM Clobbering levels on a dotted galaxy path. Learn via tutorials and quizzes and earn stars to unlock the next planet."
        canonical="/game/dom-clobbering"
        jsonLd={{
          "@context": "https://schema.org",
          "@type": "VideoGame",
          name: "DOM Clobbering World",
      ...
      <section
        ...
        style={{ height: "1400px" }}
        aria-label="DOM Clobbering galaxy map"
      >
        <img
          src={bg}
          alt="Purple-gray galaxy background with nebula"
          className="absolute inset-0 h-full w-full object-cover"
          loading="eager"
        />
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_30%_20%,hsl(var(--muted)/0.15),transparent_40%),radial-gradient(circle_at_70%_80%,hsl(var(--border)/0.15),transparent_40%)]" />
...
export default DOMClobberingMap;