...
const CSSInjectionMap = () => {
  const { progress, awardStarsAndUnlock } = useWorldProgress("cssinjection");
  ...
  const handleLevelClick = (level, locked) => {
    ...
    window.location.href = `/quiz?vulnerability=cssinjection&level=${level}`;
  };

  return (
    <main className="min-h-screen w-full">
      <SEO
        title="CSS Injection World – Cybersecurity Galaxy Game"
        description="Play through 10 CSS Injection levels on a dotted galaxy path. Learn via tutorials and quizzes and earn stars to unlock the next planet."
        canonical="/game/css-injection"
        jsonLd={{
          "@context": "https://schema.org",
          "@type": "VideoGame",
          name: "CSS Injection World",
      ...
      <section
        ...
        style={{ height: "1400px" }}
        aria-label="CSS Injection galaxy map"
      >
        <img
          src={bg}
...
export default CSSInjectionMap;