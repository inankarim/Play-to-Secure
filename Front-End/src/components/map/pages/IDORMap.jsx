...
const IDORMap = () => {
  const { progress, awardStarsAndUnlock } = useWorldProgress("idor");
  ...
  const handleLevelClick = (level, locked) => {
    ...
    window.location.href = `/quiz?vulnerability=idor&level=${level}`;
  };

  return (
    <main className="min-h-screen w-full">
      <SEO
        title="IDOR World – Cybersecurity Galaxy Game"
        description="Play through 10 IDOR levels on a dotted galaxy path. Learn via tutorials and quizzes and earn stars to unlock the next planet."
        canonical="/game/idor"
        jsonLd={{
