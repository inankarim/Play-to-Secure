...
const CSPBypassMap = () => {
  const { progress, awardStarsAndUnlock } = useWorldProgress("cspbypass");
  ...
  const handleLevelClick = (level, locked) => {
    ...
    window.location.href = `/quiz?vulnerability=cspbypass&level=${level}`;
  };

  return (
    <main className="min-h-screen w-full">
      <SEO
        title="CSP Bypass World – Cybersecurity Galaxy Game"
        description="Play through 10 CSP Bypass levels on a dotted galaxy path. Learn via tutorials and quizzes and earn stars to unlock the next planet."
        canonical="/game/csp-bypass"
        jsonLd={{
          "@context": "https://schema.org",
          "@type": "VideoGame",
          name: "CSP Bypass World",
      ...
      <section
        ...
        style={{ height: "1400px" }}
        aria-label="CSP Bypass galaxy map"
      >
        <img
          src={bg}
